    ...
    <main class="max-w-5xl mx-auto w-full px-4 -mt-16 relative z-20 flex-1 pb-16">
        ...
        <div class="grid gap-4 sm:gap-6">
            <?php if(count($careers) > 0): ?>
                ...
                <div class="bg-white rounded-3xl border border-gray-200 p-5 sm:p-8 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all group">
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-3 mb-4">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-gray-100 text-gray-700 rounded-lg text-xs font-bold font-mono">
                                    <span>📍</span> <?php echo htmlspecialchars($career['location'] ?? 'Purnea'); ?>
                                </span>
                                <?php if(!empty($career['salary'])): ?>
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-lg text-xs font-bold font-mono">
                                        <span>💰</span> <?php echo htmlspecialchars($career['salary']); ?>
                                    </span>
                                <?php endif; ?>
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-beige-50 text-beige-700 border border-beige-100 rounded-lg text-xs font-bold font-mono">
                        ...
                        <div class="shrink-0 flex items-center justify-start sm:justify-end w-full sm:w-auto border-t sm:border-0 border-gray-100 pt-4 sm:pt-0 mt-2 sm:mt-0">
                            <a 
                                ...
                                class="w-full sm:w-auto text-center px-6 py-3 bg-slate-900 hover:bg-black text-white rounded-xl text-sm font-black transition-all active:scale-95"
                            >
                                Apply Now
                            </a>
                        </div>
